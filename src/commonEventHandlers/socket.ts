...
function lobbyListEvent(payload: any) {
  const { socketId, data } = payload;
  let tableId = EMPTY;
  const responseData: responseData = {
    eventName: EVENTS.LOBBY_LIST_DETAIL,
    data
  };
  sendEventToClient(socketId, responseData, tableId);
}
...
CommonEventEmitter.on(EVENTS.LOBBY_LIST_DETAIL, lobbyListEvent);
...
CommonEventEmitter.on(PLAYER_REJOIN_TIMER_EXPIRED, async (res) => {
  ...
  if (tableGamePlay.tableStatus !== TABLE_STATE.DECLARED) {
    // leaveTableHandler({ id: socket, tableId: tableId, userId: userId }, { userId, tableId, currentRound: NUMERICAL.ONE });
  }
});
...
async function roundTimerEnd(payload: any) {
  Logger.info('roundTimerEnd. 111 : ',payload);
  let { timer, jobId, tableId, currentRound } = payload;
  await Scheduler.addJob.lockTimerStart({ timer, jobId, tableId, currentRound });
}

CommonEventEmitter.on(WAITING_FOR_PLAYER_TIMER_EXPIRED, async (res) => {
  const { tableId, tableConfig, tableGamePlay } = res;
