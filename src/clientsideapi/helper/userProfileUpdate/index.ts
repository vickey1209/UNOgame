...
import { UserProfileDataInput } from "../../../interfaces/userProfile";
...
async function userProfileUpdate(userDetail: findUserI, socketId: string):Promise<any>{
    ...
    try {
        const userOwnProfile = userDetail;//await userProfileCache.getUserProfile(userId);//await getUserOwnProfile(userDetail.authToken, socketId, userId);
        ...
        const balance : number = 0//userOwnProfile.bonus + userOwnProfile.winCash + userOwnProfile.cash || 0;
