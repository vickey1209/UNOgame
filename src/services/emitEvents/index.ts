...
const { USER_TURN_TIMER, GAME_START_TIMER, WAIT_FOR_OTHER_PLAYER_TIMER, SECONDARY_TIMER } = getConfig();
...
const emitJoinTableEvent = async (
  ...
  try {
    ...
    if (tableConfig.activePlayer > NUMERICAL.TWO) {
      ...
      else if (tableGamePlay.tableStatus === TABLE_STATE.WAIT_FOR_OTHER_PLAYERS) {
        totalRoundTimer =
          diffSeconds(new Date(), new Date(tableGamePlay.updatedAt)) * NUMERICAL.THOUSAND;
        Logger.info(tableId," before :: totalRoundTimer :: ", totalRoundTimer);
    ...
    const addJoinTableData = null;//await formatJoinTableSeats(formatedJoinTableResponse, tableConfig.noOfPlayer,tableId)
    ...
    let formatedJTResponse: any /*formatedJTResponse */= {
      tableId,
      ...
      currentTurn: tableGamePlay.currentTurnUserId,
