...
import { EVENTS, PLAYER_STATE, TABLE_STATE, NUMERICAL, MESSAGES, EMPTY } from '../../constants';
import {
  formatGameTableData
...
async function iamBack(
  ...
  try {
    ...
    playerGamePlay.userStatus = EMPTY;
    await playerGamePlayCache.insertPlayerGamePlay(playerGamePlay, tableId);   

    const tableState: string = tableGamePlay?.tableStatus;
