...
// import UserProfile from "../../mongoDB/query";
  ...
  try {
    ...
    if (!tableId) {
      ...
      if (getBlockUserArr && getBlockUserArr.tableId.length > NUMERICAL.ZERO) {
        ...
        await redis.setValueInKey(blockUserKey, { tableId: [] });
      }
      ...
      // if (!blockUserData.isNewTableCreated) {
        ...
        //   if (rediusCheckData && rediusCheckData.isGameRadiusLocationOn && rangeRediusCheck != NUMERICAL.ZERO && signUpData.isBot == false) {
            ...
            if (getLocationArr && getLocationArr.tableId.length > NUMERICAL.ZERO) {
              ...
              await redis.setValueInKey(locationKey, { tableId: [] });
            }
