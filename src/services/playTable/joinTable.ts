  ...
  try {
    if (response && 'tableId' in response) {
      ...
      if (!reconnect) {
        ...
        Logger.info(tableId, " tableGamePlay.tableState ==> :: ", tableGamePlay.tableStatus);
        ...
        // tableGamePlayInfo.seats.filter((seat) => {
        ...
        if (PGP?.userStatus == PLAYER_STATE.PLAYING) {
          ...
          if (tableConfig.noOfPlayer === NUMERICAL.FOUR || tableConfig.noOfPlayer === NUMERICAL.SIX) {
            ...
            if (totalPlayersCount === NUMERICAL.TWO) {
              ...
              scheduler.addJob.waitingForPlayerTimerStart({
              ...
              await scheduler.cancelJob.cancelWaitingForPlayerTimer(`waitingForPlayerTimer:${tableId}`, tableId);
