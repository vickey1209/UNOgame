...
import { EMPTY, EVENTS, MESSAGES, MONGO, NUMERICAL, PLAYER_STATE, TABLE_STATE } from "../../constants";
import { NewGTIResponse, RejoinResponse } from "../../interfaces/tableConfig";
import { formatRejoinTableData, formatSignUpData } from "../../formatResponseData";
...
async function reconnection(
  ...
  try {
    ...
    if (!tableConfig) {
      throw new Errors.UnknownError('table config not found !');
    }

    if (!tableGamePlay)
      throw new Errors.UnknownError('tableGamePlay not found !');

    if (!playerGamePlay)
      throw new Errors.InvalidInput('playerGamePlay not found !');
    ...
    console.log("userProfile.tableIds ::==>>", userProfile.tableIds);
